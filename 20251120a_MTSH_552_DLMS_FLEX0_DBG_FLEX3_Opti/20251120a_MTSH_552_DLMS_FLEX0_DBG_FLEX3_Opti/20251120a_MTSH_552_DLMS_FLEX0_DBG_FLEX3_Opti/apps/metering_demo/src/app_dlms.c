...
{
    app_dlmsData.dlms_uart_rx_buf[app_dlmsData.dlms_uart_rx_buf_idx++] = app_dlmsData.dlms_uart_rx_byte;
    
    if(app_dlmsData.dlms_uart_rx_buf_idx >= UART_TXRX_BUF_SIZE)
        app_dlmsData.dlms_uart_rx_buf_idx = 0;
...
void APP_DLMS_Initialize ( void )
{
    ...
    app_dlmsData.dlms_uart_rx_buf_idx = 0;
    app_dlmsData.dlms_uart_rx_buf_gurux_idx = 0;    

    FLEXCOM0_USART_ReadCallbackRegister(_APP_DLMS_UART_ReadCallback, 0);
    FLEXCOM0_USART_Read(&app_dlmsData.dlms_uart_rx_byte, 1);
    
    FLEXCOM0_USART_Write("DLMS RS485 OK\r\n", sizeof ("DLMS RS485 OK\r\n"));
...
void APP_DLMS_Tasks ( void )
{
    ...
    switch ( app_dlmsData.state )
    {
        ...
        case APP_DLMS_STATE_SERVICE_TASKS:
        {
            ...
            if ( app_dlmsData.dlms_uart_rx_buf_gurux_idx != app_dlmsData.dlms_uart_rx_buf_idx)
            {
                dlms_usart_data_cb(1, &app_dlmsData.dlms_uart_rx_buf[app_dlmsData.dlms_uart_rx_buf_gurux_idx]);
                
                app_dlmsData.dlms_uart_rx_buf_gurux_idx++;
                if(app_dlmsData.dlms_uart_rx_buf_gurux_idx >= UART_TXRX_BUF_SIZE)
                    app_dlmsData.dlms_uart_rx_buf_gurux_idx = 0;
            }
