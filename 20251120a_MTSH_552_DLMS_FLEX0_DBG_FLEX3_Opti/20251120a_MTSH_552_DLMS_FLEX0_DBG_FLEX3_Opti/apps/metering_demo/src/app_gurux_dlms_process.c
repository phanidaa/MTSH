...
#define GURUX_DLMS_PROCESS_USART_Write  FLEXCOM3_USART_Write
...
// Define transport layer type - uncomment one
//#define USE_HDLC
#define USE_WRAPPER
...
#ifdef USE_WRAPPER
    //Buffer where frames are saved (Wrapper).
...
#else // USE_HDLC
    //Buffer where frames are saved (HDLC).
 ...
 * Association Objects
*/
static gxAssociationLogicalName associationNone;        /* 0.0.43.0.1.255 - Client SAP = 16; Server SAP = 1 */
static gxAssociationLogicalName associationLow;         /* 0.0.43.0.2.255 - Client SAP = 17; Server SAP = 1 */
static gxAssociationLogicalName associationHigh;        /* 0.0.40.0.3.255 - Client SAP = 18; Server SAP = 1 */
...
gxScriptTable scriptTableMyAction;
...
// Forward declarations for profile functions (commented-out functions need declaration)
int obj_instantaneouRegistersProfile(void);
...
int addscriptTableMyAction() {
    int ret;
    static gxScript SCRIPTS[1] = { 0 };
    const unsigned char ln[6] = { 0, 0, 10, 0, 101, 255 };  // OBIS code
    
    if ((ret = INIT_OBJECT(scriptTableMyAction, 
                           DLMS_OBJECT_TYPE_SCRIPT_TABLE, ln)) == 0) {
        SCRIPTS[0].id = 1;
        ARR_ATTACH(scriptTableMyAction.scripts, SCRIPTS, 1);
    }
    return ret;
}
...
int loadSettings()
{
    GURUX_DLMS_PROCESS_USART_Write("loadSettings\r\n", sizeof ("loadSettings\r\n"));
    int ret = 0;
...
///////////////////////////////////////////////////////////////////////
//This method adds example Logical Name Association object for High SHA256 authentication.
// UA in Indian standard.
///////////////////////////////////////////////////////////////////////
int addAssociationHighSHA256()
{
    ...
    if ((ret = INIT_OBJECT(associationHighSHA256, DLMS_OBJECT_TYPE_ASSOCIATION_LOGICAL_NAME, ln)) == 0)
    {
        ...
        associationHighSHA256.clientSAP = 0x1;
        ...
        associationHighSHA256.xDLMSContextInfo.conformance = (DLMS_CONFORMANCE)(DLMS_CONFORMANCE_BLOCK_TRANSFER_WITH_ACTION |
            DLMS_CONFORMANCE_BLOCK_TRANSFER_WITH_SET_OR_WRITE |
            DLMS_CONFORMANCE_BLOCK_TRANSFER_WITH_GET_OR_READ |
            DLMS_CONFORMANCE_SET |
            DLMS_CONFORMANCE_SELECTIVE_ACCESS |
...
#ifndef DLMS_IGNORE_OBJECT_POINTERS
        associationHighSHA256.securitySetup = &securitySetupHigh;
#else
        memcpy(associationHighSHA256.securitySetupReference, securitySetupHigh.base.logicalName, 6);
#endif //DLMS_IGNORE_OBJECT_POINTERS
    }
    return ret;
}
...
int addSecuritySetupHigh(void)
{
    int ret;
    const unsigned char ln[6] = { 0, 0, 43, 0, 2, 255 }; // Security Setup (class 64)
 
    // >>> Replace these with your real 8-byte titles (MUST match your client & meter docs)
    static const unsigned char SERVER_ST[8] = { 0x70,0x38,0x9C,0xCE,0x67,0xB3,0x59,0xAC }; // example
    static const unsigned char CLIENT_ST[8] = { 0x41,0x42,0x43,0x44,0x45,0x46,0x47,0x48 }; // "ABCDEFGH" example
    // <<<
 
    if ((ret = INIT_OBJECT(securitySetupHigh, DLMS_OBJECT_TYPE_SECURITY_SETUP, ln)) != 0)
        return ret;
 
    // Suite 0 is fine here; SHA-256 HLS is authentication only (not ciphering).
    securitySetupHigh.securitySuite = DLMS_SECURITY_SUITE_V0;
    
    bb_clear(&securitySetupHigh.serverSystemTitle);
    bb_clear(&securitySetupHigh.clientSystemTitle);
    bb_set(&securitySetupHigh.serverSystemTitle, SERVER_ST, sizeof(SERVER_ST));
    bb_set(&securitySetupHigh.clientSystemTitle, CLIENT_ST, sizeof(CLIENT_ST));
    return 0;
}
...
int addKek()
{
    ...
    {
        GX_OCTET_STRING(kek.value, uartSettings.base.cipher.blockCipherKey, 16);
    }
    return ret;
}
...
///////////////////////////////////////////////////////////////////////
//Add profile generic for instantaneous values (not used - comment out)
///////////////////////////////////////////////////////////////////////
int obj_instantaneouRegistersProfile()
{
    int ret;
    static gxTarget CAPTURE_OBJECT[10] = { 0 };
...
int readProfileGeneric(dlmsSettings* settings, gxProfileGeneric* pg, unsigned char type, gxValueEventArg* e)
{
    ...
    ARR_ATTACH(captureObjects, CAPTURE_OBJECT, 10);
    ...
    if (first)
    {
        ...
        else if (e->selector == 1)
        {
            ...
            if (pg->captureObjects.size == 0 || pg->captureObjects.data == NULL)
            {
                ret = DLMS_ERROR_CODE_HARDWARE_FAULT;
                e->transactionStartIndex = e->transactionEndIndex = 0;
            }
            else if ((ret = getProfileGenericDataByRangeFromRingBuffer(settings, type, e)) != 0 ||
        ...
        else if (e->selector == 2)
        {
            ...
            else if ((ret = cosem_checkStructure(e->parameters.byteArr, 4)) != 0 ||
...
//        }
        ...
        else if (e->target == BASE(scriptTableMyAction)) {
            // Do your action here
            GXTRACE_LN(GET_STR_FROM_EEPROM("svr_preRead: scriptTableMyAction triggered"), e->target->objectType, e->target->logicalName);
            e->handled = 1;
        }
    }
}
...
void svr_preAction(
    ...
    gxValueEventCollection* args)
{
    GURUX_DLMS_PROCESS_USART_Write("svr_preAction\r\n", sizeof ("svr_preAction\r\n")); 
    ...
    for (pos = 0; pos != args->size; ++pos)
    {
        ...
        else if (e->target == BASE(disconnectControl))
        {
            ...
            if (e->index == 1)
            {
                ...
                GURUX_DLMS_PROCESS_USART_Write("Disconnection\r\n", sizeof ("Disconnection\r\n")); 
            }
            else //Reconnnect. Turn LED ON.
            {
                GURUX_DLMS_PROCESS_USART_Write("Reconnection\r\n", sizeof ("Reconnection\r\n")); 
            }
        }
...
void svr_postAction(
    ...
    gxValueEventCollection* args)
{
    GURUX_DLMS_PROCESS_USART_Write("svr_postAction\r\n", sizeof ("svr_postAction\r\n")); 
    ...
    for (pos = 0; pos != args->size; ++pos)
    {
        if ((ret = vec_getByIndex(args, pos, &e)) != 0)
        {
            GURUX_DLMS_PROCESS_USART_Write("vec_getByIndex return\r\n", sizeof ("vec_getByIndex return\r\n")); 
            return;
        }
        ...
        if ((action = svr_isChangedWithAction((DLMS_OBJECT_TYPE)e->target->objectType, e->index)) != 0)
        {
            ...
            if (e->error == 0)
            {
                GURUX_DLMS_PROCESS_USART_Write("svr_postAction->do nothing\r\n", sizeof ("svr_postAction->do nothing\r\n")); 
                //saveSettings(e->target, action);
            }
            else
            {
                GURUX_DLMS_PROCESS_USART_Write("svr_postAction->loadSettings\r\n", sizeof ("svr_postAction->loadSettings\r\n")); 
                //Load default settings if there is an error.
                loadSettings();
            }
        }
        else
            GURUX_DLMS_PROCESS_USART_Write("svr_isChangedWithAction->==0\r\n", sizeof ("svr_isChangedWithAction->==0\r\n")); 
    }
}
...
DLMS_ACCESS_MODE getDisconnectControlAttributeAccess(
    ...
    unsigned char index)
{
    // Allow writing scripts (attribute 2) only for High authentication.
    if (index == 2) {
        if (settings->authentication > DLMS_AUTHENTICATION_LOW) {
            return DLMS_ACCESS_MODE_READ_WRITE;
        } else {
    ...
    // Other attributes remain read-only.
    return DLMS_ACCESS_MODE_READ;
}
...
void init_dlms_process(void)
{
    ...
    SYS_CONSOLE_Print(SYS_CONSOLE_INDEX_0,"DLMS INIT Done\r\n");
}