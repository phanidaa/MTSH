...
//    UART_Read(&app_optouartData.optical_rx_byte,1 );
    ...
    app_optouartData.dlms_uart_rx_buf[app_optouartData.dlms_uart_rx_buf_idx++] = app_optouartData.dlms_uart_rx_byte;
    
    if(app_optouartData.dlms_uart_rx_buf_idx >= OPTICAL_UART_TXRX_BUF_SIZE)
        app_optouartData.dlms_uart_rx_buf_idx = 0;
    
    UART_Read(&app_optouartData.dlms_uart_rx_byte, 1);
}
...
void APP_OPTOUART_Initialize ( void )
{
    ...
    app_optouartData.dlms_uart_rx_buf_idx = 0;
    app_optouartData.dlms_uart_rx_buf_gurux_idx = 0;    
...
void APP_OPTOUART_Tasks ( void )
{
    ...
    switch ( app_optouartData.state )
    {
        ...
        {
            if ( app_optouartData.dlms_uart_rx_buf_gurux_idx != app_optouartData.dlms_uart_rx_buf_idx)
            {
                dlms_usart_data_cb(1, &app_optouartData.dlms_uart_rx_buf[app_optouartData.dlms_uart_rx_buf_gurux_idx]);
                
                app_optouartData.dlms_uart_rx_buf_gurux_idx++;
                if(app_optouartData.dlms_uart_rx_buf_gurux_idx >= OPTICAL_UART_TXRX_BUF_SIZE)
                    app_optouartData.dlms_uart_rx_buf_gurux_idx = 0;
            }
